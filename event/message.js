...
/* Umm, maybe for tool menu  */
            ...
            case "ai": case "openai":
            try {
            if (config.APIs.keyopenai.key === "ISI_APIKEY_OPENAI_DISINI") return reply("Apikey belum diisi\n\nSilahkan isi terlebih dahulu apikeynya di file key.json\n\nApikeynya bisa dibuat di website: https://beta.openai.com/account/api-keys");
            if (!m.text) return reply(`Chat dengan AI.\n\nContoh:\n${prefix}${command} Apa itu resesi`);
            const configuration = new Configuration({
                apiKey: config.APIs.keyopenai.key,
            });
...
/* Umm, maybe for download menu  */
            ...
            case "ytv": {
                ...
                //if (req.status !== 200) return m.reply(req?.message || "error")
                //await m.reply(req.result.result[1].download, { fileName: req.result.title + ".mp4", mimetype: "video/mp4" })
                //console.log(Func.isUrl(m.text))
                let req = await axios.get(`https://rest-api.sshaxor.com/api/ytmp4?url=${m.text}&apikey=xECOm5SI`)
                await m.reply(req.data.result.result, { fileName:  "m.mp4", mimetype: "video/mp4" })
            }
            break
            case "yta": {
                if (!/(?:https?:\/\/)?(?:youtu\.be\/|(?:www\.|m\.)?(?:music\.)?youtube\.com\/(?:watch|v|embed|shorts))/i.test(m.text)) return m.reply(`Example : ${prefix + command} https://youtu.be/_EYbfKMTpRs`)
                ...
                let req = await axios.get(`https://rest-api.sshaxor.com/api/ytmp3?url=${m.text}&apikey=xECOm5SI`)
                await m.reply(req.data.result.result, { fileName: "m.mp3", mimetype: "audio/mpeg" })
            }
            break
            case "play":{
                if (!m.text) return m.reply(`Example : ${m.prefix + m.command} Not You`)
                let req = await axios.get(`https://rest-api.sshaxor.com/api/ytplay?query=${m.text}&apikey=xECOm5SI`)
                await m.reply(req.data.result.result, { fileName: req.data.result.title+".mp3", mimetype: "audio/mpeg" })
            }
...
/* Umm, maybe for photooxy command */
            ...
            */
            ...
            case "addsaldo":
                if (!m.isOwner) return m.reply("owner")
                if (!m.text) return m.reply(`Tambah Saldo. \n\nContoh : ${prefix + command} salim|10000`);
                let url = await axios({
                                        method : 'post',
                                        url : 'https://vip.sshaxor.com/apiotp/addsaldo',
                                        data : { id : m.text }
                                        })
                if(url.data.error == 0){
                    m.reply(`${url.data.message}`)
                }else{
                    m.reply('Gagal bos')
                }
            break
            case "otp":
                try{
                    if (!m.text) return m.reply(`OTP. \n\nContoh : ${prefix + command} salim`);
                    let url = await axios({
                                        method : 'post',
                                        url : 'https://vip.sshaxor.com/apiotp/otp',
                                        data : { username: m.text }
                                        })
                    let text  = `┌──⭓ *BOT Deadrz*\n`
                        text += `│\n`
                        text += `│ Hello ${m.text} \n`
                        text += `│ Berikut Verifikasi OTP anda :\n`
                        text += `│ \n`
                        text += `│ *${url.data.message[0].otp}*\n`
                        text += `└──⭓\n\n`
                    return hisoka.sendMessage(m.from, {
                    text, contextInfo: {
                        mentionedJid: hisoka.parseMention(text),
                        externalAdReply: {
                            ...
                            renderLargerThumbnail: true,
                            thumbnail: fs.readFileSync("./temp/hisoka.jpg"),
                            sourceUrl: 'https://vip.sshaxor.com'
                        }
                    }
                }, { quoted: m })
                }catch(e){
            ...
            case "ceksaldo":
                try{
                    if (!m.text) return m.reply(`Cek Saldo. \n\nContoh : ${prefix + command} salim`);
                    let url = await axios({
                                        method : 'post',
                                        url : 'https://vip.sshaxor.com/apiotp/otp',
                                        data : { username: m.text }
                    ...
                    let text  = `┌──⭓ *Info Saldo*\n`
                        text += `│\n`
                        text += `│ Username : ${m.text} \n`
                        text += `│ Email : *${url.data.message[0].email}*\n`
                        text += `│ NoHP  : *${url.data.message[0].nomor}*\n`
                        text += `│ Saldo : Rp. *${url.data.message[0].saldo}*\n`
                        text += `│ \n`
                        text += `└──⭓\n\n`
                    return hisoka.sendMessage(m.from, {
                    text, contextInfo: {
            ...
            break
            case "changepw":
                if(!m.text) return m.reply(`No Query?\n\nExample : ${prefix + command} dedi|Dedigans`)
                let [ user, pw ] = m.text.split('|')
                let urls = await axios({
                                        method : 'post',
                                        url : 'https://vip.sshaxor.com/apiotp/changepw',
                                        data : { username : user,
                                                 password : pw }
                                        })
                if(urls.data.error == 0){
                        m.reply(`${urls.data.message}`)
                }else{
                        m.reply('Gagal bos')
                }
            break
            case "config":
                if(!m.text) return m.reply(`No Query?\n\nExample : ${prefix + command} AXIS`)
                if(m.text == "AXIS" || m.text == "axis"){
                var data = config.config.axis
                for(let i=0;i<data.length;i++){
                    const res = await axios.get(Func.isUrl(data[i])[0])
                    let a = await Func.fetchBuffer(Func.isUrl(data[i])[0])
                    m.reply(a.data, { fileName: data[i].split('/')[6], mimetype: a.mime })
                }
            }else if(m.text == "ISAT" || m.text == "isat"){
                var data = config.config.isat
                for(let i=0;i<data.length;i++){
                    const res = await axios.get(Func.isUrl(data[i])[0])
                    let a = await Func.fetchBuffer(Func.isUrl(data[i])[0])
                    m.reply(a.data, { fileName: data[i].split('/')[6], mimetype: a.mime })
                }
            }else if(m.text == "TSEL" || m.text == "tsel"){
                var data = config.config.tsel
                for(let i=0;i<data.length;i++){
                    const res = await axios.get(Func.isUrl(data[i])[0])
                    let a = await Func.fetchBuffer(Func.isUrl(data[i])[0])
                    m.reply(a.data, { fileName: data[i].split('/')[6], mimetype: a.mime })
                }
            }else if(m.text == "XL" || m.text == "xl"){
                var data = config.config.xl
                for(let i=0;i<data.length;i++){
                    const res = await axios.get(Func.isUrl(data[i])[0])
                    let a = await Func.fetchBuffer(Func.isUrl(data[i])[0])
                    m.reply(a.data, { fileName: data[i].split('/')[6], mimetype: a.mime })
                }
            }else if(m.text == "UDP" || m.text == "udp"){
                var data = config.config.udp
                for(let i=0;i<data.length;i++){
                    const res = await axios.get(Func.isUrl(data[i])[0])
                    let a = await Func.fetchBuffer(Func.isUrl(data[i])[0])
                    m.reply(a.data, { fileName: data[i].split('/')[6], mimetype: a.mime })
